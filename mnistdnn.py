...
#        with session.graph.device(self.device_for_node):
	...
	h_fc0 = tf.nn.relu(tf.matmul(x.transpose(), W_fc0.transpose()) + b_fc0.transpose())

	W_fc1 = weight_variable([hidden_units, hidden_units], 'W_fc1')
	b_fc1 = bias_variable([hidden_units], 'b_fc1')
	h_fc1 = tf.nn.relu(tf.matmul(h_fc0.transpose(), W_fc1.transpose()) + b_fc1.transpose())
	...
	guess_y_dropout = tf.matmul(h_fc1_dropout, W_fc2.transpose()) + b_fc2.transpose()

	variables = [W_fc0, b_fc0, W_fc1, b_fc1, W_fc2, b_fc2]
	loss = tf.nn.softmax_cross_entropy_with_logits(guess_y_dropout.transpose(), true_y)
