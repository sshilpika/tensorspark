...
class MnistDNN(ParameterServerModel):
    ...
    #        with session.graph.device(self.device_for_node):
    	...
    	h_fc0 = tf.nn.relu(tf.matmul(x, W_fc0,transpose_a=False, transpose_b=False) + b_fc0)

    	W_fc1 = weight_variable([hidden_units, hidden_units], 'W_fc1')
    	b_fc1 = bias_variable([hidden_units], 'b_fc1')
    	h_fc1 = tf.nn.relu(tf.matmul(h_fc0, W_fc1,transpose_a=False, transpose_b=False) + b_fc1)
    	...
    	guess_y = tf.matmul(h_fc1, W_fc2,transpose_a=False, transpose_b=False) + b_fc2
    	guess_y_dropout = tf.matmul(h_fc1_dropout, W_fc2,transpose_a=False, transpose_b=False) + b_fc2
